...
public class Spawner : MonoBehaviour
{
    ...
    private IEnumerator Spawn()
    {
        ...
        while(enabled)
        {
            ...
            GameObject mask = Instantiate(prefab, position, rotation);

            Destroy(mask, maxLifeTime);

            float force = Random.Range(minForce, maxForce);

            mask.GetComponent<Rigidbody2D>().AddForce(mask.transform.up * force, ForceMode2D.Impulse);
